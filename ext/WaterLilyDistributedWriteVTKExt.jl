module WaterLilyDistributedWriteVTKExt
...
using WaterLily: default_attrib, pvd_collection, inside, sim_time
using WaterLilyDistributed
import WaterLilyDistributed: vtkWriter_d, write_d!
...
function vtkWriter_d(fname="WaterLily";attrib=default_attrib(),dir="vtk_data",T=Float32,extents=[(1:1,1:1)])
    (WaterLilyDistributed.master() && !isdir(dir)) && mkdir(dir)
    VTKWriter_d(fname,dir,pvd_collection(fname),attrib,[0],extents)
end
function vtkWriter_d(fname,dir::String,collection,attrib::Dict{String,Function},k,extents)
    VTKWriter_d(fname,dir,collection,attrib,[k],extents)
end
...
"""
# parallel version of that file
function write_d!(w::VTKWriter_d,a;N=size(inside(a.flow.p)))
...
"""
close(w::VTKWriter_d)=(vtk_save(w.collection);nothing)
"""
...
end # module WaterLilyDistributedWriteVTKExt
