...
function circle(dims,center,radius;Re=250,U=1,psolver=MultiLevelPoisson,mem=Array)
    ...
    DistributedSimulation(dims, (U,0,0), radius; ν=U*radius/Re, body, exitBC=false, mem=mem, psolver=psolver)
end
...
r = init_mpi((L,2L,2L))
sim = circle((L,2L,2L),SA[L/2,L+2,L],L/8;mem=MPIArray) #use MPIArray to use extension

wr = vtkWriter_d("WaterLily-MPI-sphere";attrib=custom_attrib,dir="vtk_data",
